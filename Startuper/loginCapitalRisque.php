...
if (isset($_POST['submit'])){
   ...
   // Vérification de l'existence de l'email dans la base de données
    $select_capitalrisque = $conn->prepare("SELECT * FROM capital_risque WHERE pseudo_cr=? AND pwrd_cr=?");
    $select_capitalrisque->execute([$pseudo, $pwrd]);

    if ($select_capitalrisque->rowCount()>0){
        session_start(); // Démarrage de la session
    $row = $select_capitalrisque->fetch(PDO::FETCH_ASSOC);
    //STORE ID IN SESSION
    $_SESSION['id_capital_risque'] = $row['id_capital_risque'];
    header('location: ProfilCapitalRisque.html');
    exit();
} else {
...
<head>
    ...
    <title>loginCapitalRisque Page</title>
...
<div class="form-container">
    ...
    <form id="form" name ="form" action="" method="post" enctype="multipart/form-data" >
        ...
        <p class="link">Do not have an account ? <a href="registerCapitalRisque.php">Register Now</a></p>
        <!--BUTTON LOGIN NOW-->
